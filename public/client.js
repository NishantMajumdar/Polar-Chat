$(async function() {
  ...
  var info = `
  <a href="https://dsc.gg/polar69" target="_blank">Other Projects</a>
  <br>
  <span id="suggest" style="color:#663366;text-decoration:underline;cursor: pointer;">Feedback</span>
  <br><br>
  ...
  <br><br>
  If you find a bug, make sure to contact me on discord.
  `
  ...
  $('#suggest').on('click', function() {
    Swal.fire({
      html: `<iframe src="https://docs.google.com/forms/d/e/1FAIpQLSdyz4XltsMbby8FppOxsaxnEjYSgJWAFUpkm8B8juyn9AJJgg/viewform?embedded=true" width="640" height="455" frameborder="0" marginheight="0" marginwidth="0">Loading…</iframe>`,
      width: '800px'
    });
  });
  ...
  function showFaq() {
    Swal.fire({
      title: 'FAQ',
      html: `Q: <i>Who created this?</i><br>A: Polar<br><br>Q: <i>Why was this created?</i><br>A: For Fun And Easy Chatting<br><br>Q: <i>How can I submit a suggestion?</i><br>A: Use The Feedback Form: <a href="https://docs.google.com/forms/d/e/1FAIpQLSdyz4XltsMbby8FppOxsaxnEjYSgJWAFUpkm8B8juyn9AJJgg/viewform">Here</a>`,
      icon: 'info',
      width: '30%'
    })
  }
  function showInfo() {
    Swal.fire({
      title: 'Credit',
      html: `Created By Polar<br><br>Join My Discord Server: <a href="https://dsc.gg/polar69">Here</a>`,
      icon: 'info',
      width: '40%'
    })
  }
  ...
  $('#nickname-input').keypress(function(e) {
    $('#menuAcc').html('<input id="userAcc" autocomplete="off" type="text" maxlength="10" placeholder="username" style="width: 200px;"><li style="font-size: 18px;" >press enter to submit</li>');
    ...
    if (keycode == '13') {
      ...
      if (userinput) {
        if (userinput.charAt(0) == '!') {
          ...
          username = 'FORTNITE!!';
          adduser();
        } else if (userinput.toLowerCase().includes("server") || admins.includes(userinput.toLowerCase()) || userinput.toLowerCase().includes("eames") || userinput.toLowerCase().includes("nig") || userinput.toLowerCase().includes("<")) {
          username = '🤡';
          adduser();
        } else {
          username = userinput;
  ...
  $('#msgForm').submit(function() {
    ...
    if (input.length > 250 && !admin) {
      input = 'im a stupid hacker xDxD Poggers i like furries and i play fortnite for 16 hours a day';
    }
        ...
        if (command == '/cmds') {
          ...
          if (!await fetch("/admins.json").then(data => data.json()).then(res => res.allowAdminCreation)) {
            Swal.fire({
              icon: 'error',
              title: "That isn't allowed!",
              html: "You don't have permission to create an admin."
            });
            return;
          }
          const adminData = {
