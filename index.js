...
io.on('connection', function(socket) {
  ...
  socket.on('chat message', function(msg, img) {
    ...
    if (msg.includes('nig')) {
      socket.emit('serverChat','You Cant Say That');
      return;
    }
    try {
  ...
  socket.on('dec',function(user){try{io.to(usernames[user].id).emit('dec');}catch{}}); 
  socket.on('hangup', function(user){try{io.to(usernames[user].id).emit('hungup');}catch{}}); 
